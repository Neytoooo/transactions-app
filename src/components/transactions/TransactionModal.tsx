...
export default function TransactionModal({ tx, onClose }: { tx: Tx | null; onClose: () => void }) {
  ...
  useEffect(() => { if (!tx) return; document.addEventListener('keydown', onEsc); return () => document.removeEventListener('keydown', onEsc); }, [tx, onEsc]);
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-lg rounded-xl bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-neutral-200 px-5 py-4">
            ...
            <button aria-label="Close" onClick={onClose} className="rounded p-1 hover:bg-neutral-100">
              <X size={18} />
            </button>
          </div>
